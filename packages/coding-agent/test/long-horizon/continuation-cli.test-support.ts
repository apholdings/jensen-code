 ...
 *
 * Real child-process tests that invoke the actual CLI through the installed
 * tsx entry point.
 */
...
function runCli(args: string[]): { stdout: string; stderr: string; status: number | null } {
	const result = spawnSync(
		process.execPath,
		[TSX_CLI, resolve(REPO_ROOT, "packages", "coding-agent", "src", "cli.ts"), ...args],
		{
			cwd: REPO_ROOT,
			timeout: 15_000,
			encoding: "utf-8",
...
export function registerInitTests(): void {
...
export function registerInspectTests(): void {
		...
		it("reports ENOENT for missing scheduler", () => {
			...
			try {
				const result = runCli([
					...
					"inspect",
					"--scheduler",
					resolve(dir, "nonexistent-scheduler.json"),
				]);
				expect(result.status).toBe(1);
...
export function registerCancelTests(): void {
	describe("CLI cancel", () => {
...
export function registerAbandonTests(): void {
...
export function registerValidateTests(): void {
