...
/**
 * Long-horizon CLI trust-boundary contract tests (part 2 of 2).
 *
 * Covers the serialized-context forgery rejections and the SATISFIED-boundary
 * regression. Split out of the original single file so each vitest worker stays
 * well under the 60s RPC ceiling (vitest birpc DEFAULT_TIMEOUT) the many
 * full-CLI spawns can exceed under CI load. Coverage is unchanged.
 */

describe("generic CLI trust boundary (satisfied)", () => {
