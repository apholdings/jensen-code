...
- Updated dependencies [392db26]
  ...
  - @apholdings/jensen-ai@2.1.0
  - @apholdings/jensen-tui@2.1.0
...
- Updated dependencies [10549ec]
  - @apholdings/jensen-ai@2.0.2
  - @apholdings/jensen-agent-core@2.0.2
  - @apholdings/jensen-tui@2.0.2
...
- Updated dependencies [1854dad]
  - @apholdings/jensen-ai@2.0.1
  - @apholdings/jensen-agent-core@2.0.1
  - @apholdings/jensen-tui@2.0.1
...
- 138f6a8: Durable TODO conflict recovery and progress-aware loop protection
...
- Add a canonical operator roster loader (`getAgentDir()/agents/*.md`) that guarantees the investigator, tester, and synthesizer roles, loads per-file with fault-tolerant diagnostics (including malformed frontmatter), and enforces the local `llamacpp-qwen38-bucephalus/qwen3.8-27b` model in strict mode.
- Add a local Qwen orchestration planner adapter (`QwenPlannerAdapter`) that proposes machine JSON plans through the normal `createAgentSession()` session path with read-only tools; it exposes its roster and allowed agent names, and is not yet wired into the CLI or orchestrator lifecycle.
- Extend `validateOrchestrationPlan` with an explicit `operatorAgents` option so roster-only operator roles validate when the operator set is supplied; the canonical subagent registry remains the default authority.
- Add the typed parent orchestration execution contract (`MissionRequest.orchestrationExecution`: orchestration plan identity plus named child execution authority) with validation and immutable cloning, and the `OrchestrationChildExecutionPort` child execution authority port stored as an `OrchestratorService` option and exposed through its access seam; the orchestrator itself still never launches children.
- Add the `SchedulerWorkerChildExecutionPort` adapter (durable scheduling intent enqueue plus read-only child status polling over Scheduler/Assignment/Worker; it never launches) and the parent-side `OrchestrationLifecycleExecutor`, which resolves the authority named by the parent's durable contract (verified, never defaulted), drives launch passes for materialized nodes through `port.executeChild`, and polls child status through the port's new optional `childStatus` contract; parent request/CLI wiring remains separate.
- Add the real in-process `InProcessMissionExecutor` behind the `MissionExecutor` seam: it drives a `MissionRequest` through the normal `createAgentSession()` session path (real model resolved from the mission's `modelPolicy` — verified, never defaulted — real tools selected by execution mode and narrowed by declared capabilities, durable child session bound to `childSessionId`, no child process). A clean completion is PARTIAL/unverified without a verifier, SUCCEEDED only when a verifier verifies, and a runner/session failure is CRASHED with a structured launch failure; cancellation is idempotent and aborts the underlying session.
- Add deterministic Mission Governance with independent child/parent/orchestration budgets, locked durable accounting, policy precedence, unknown-cost fail-closed semantics, retry classes, result-envelope recovery, conservative parking/stagnation inputs, and selective local-Qwen-to-configured-cloud escalation that preserves Mission identity. Governance remains an admission/decision layer and never launches, assigns, acquires inference, or commits Mission success.
...
- af149b0: Adaptive long-horizon runtime (1.5.0):
...
- Add session-scoped Caveman output compression with configurable levels, safety exclusions, and natural-language controls.
...
- Fixed `@apholdings/jensen-ai/oauth` extension imports in published installs by resolving the subpath directly from built `dist` files instead of package-root wrapper shims ([#1856](https://github.com/badlogic/pi-mono/issues/1856)).
...
- Moved Node OAuth runtime exports off the top-level `@apholdings/jensen-ai` entry. OAuth login and refresh must be imported from `@apholdings/jensen-ai/oauth` ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
...
- Fixed extension loading with the new `@apholdings/jensen-ai/oauth` export path by aliasing the oauth subpath in the extension loader and development path mapping ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
...
- Extensions now load correctly in compiled Bun binary using `@mariozechner/jiti` fork with `virtualModules` support. Bundled packages (`@sinclair/typebox`, `@apholdings/jensen-tui`, `@apholdings/jensen-ai`, `@apholdings/jensen-code`) are accessible to extensions without filesystem node_modules.
...
```typescript
// Before (hook)
import type { HookAPI, HookContext } from "@apholdings/jensen-code";
export default function (pi: HookAPI) { ... }

// Before (custom tool)
import type { CustomToolFactory } from "@apholdings/jensen-code";
...
- **Key detection functions removed from `@apholdings/jensen-tui`**: All `isXxx()` key detection functions (`isEnter()`, `isEscape()`, `isCtrlC()`, etc.) have been removed. Use `matchesKey(data, keyId)` instead (e.g., `matchesKey(data, "enter")`, `matchesKey(data, "ctrl+c")`). This affects hooks and custom tools that use `ctx.ui.custom()` with keyboard input handling. ([#405](https://github.com/badlogic/pi-mono/pull/405))
...
- `Attachment` type removed. Use `ImageContent` from `@apholdings/jensen-ai` instead. Add images directly to message content arrays.
...
- **SDK API changes** ([#296](https://github.com/badlogic/pi-mono/issues/296)):
  ...
  - Use `getModel()` from `@apholdings/jensen-ai` for built-in models, `modelRegistry.find()` for custom models + built-in models
...
- **OAuth and model config exports**: Scripts using `AgentSession` directly can now import `getAvailableModels`, `getApiKeyForModel`, `findModel`, `login`, `logout`, and `getOAuthProviders` from `@apholdings/jensen-code` to reuse OAuth token storage and model resolution. ([#245](https://github.com/badlogic/pi-mono/issues/245))
...
- **`getMarkdownTheme()` export**: Custom tools can now import `getMarkdownTheme()` from `@apholdings/jensen-code` to use the same markdown styling as the main UI.
...
- Hooks loader now supports same import aliases as custom tools (`@sinclair/typebox`, `@apholdings/jensen-ai`, `@apholdings/jensen-tui`, `@apholdings/jensen-code`).
...
- Fixed custom tools failing to load from `~/.pi/agent/tools/` when pi is installed globally. Module imports (`@sinclair/typebox`, `@apholdings/jensen-tui`, `@apholdings/jensen-ai`) are now resolved via aliases.
...
- Updated `@apholdings/jensen-ai` with X-Initiator header support for GitHub Copilot, ensuring agent calls are not deducted from quota. ([#200](https://github.com/badlogic/pi-mono/pull/200) by [@kim0](https://github.com/kim0))
...
- Updated `@apholdings/jensen-ai` with interleaved thinking enabled by default for Anthropic Claude 4 models.
...
- Updated `@apholdings/jensen-ai` with interleaved thinking support for Anthropic models.
