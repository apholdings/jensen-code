/**
 * CLI malformed --expected-revision regression tests.
 *
 * Real child-process tests that invoke the actual CLI via
 * npx tsx packages/coding-agent/src/cli.ts.
 *
 * Separated from execution-cli.test.ts so the existing 15-test suite
 * stays under the 60-second Vitest RPC timeout threshold.
 */

import { spawnSync } from "child_process";
import { createHash } from "crypto";
import { mkdtempSync, readdirSync, readFileSync, rmdirSync } from "fs";
...
function countTempSiblings(dirPath: string, execPath: string): number {
	...
	for (const entry of entries) {
		...
		if (full !== stem && entry.startsWith(`${resolve(execPath).split("/").pop()!}.`) && entry.includes(".tmp.")) {
			count++;
		}
	}
	return count;
...
const MALFORMED_VALUES: ReadonlyArray<{
