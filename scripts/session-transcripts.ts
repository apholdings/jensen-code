 ...
 *   --analyze      Spawn jensen subagents to analyze each transcript file
...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("jensen", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn jensen: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".jensen/agent/sessions");
	...
	if (!analyzeFlag) {
		console.log("\nRun with --analyze to spawn jensen subagents for pattern analysis.");
		return;
	}
